...
export function Sidebar({ route, onNavigate }: { route: Route; onNavigate: (route: Route) => void }) {
  return <aside className="sidebar"><div className="brand"><span className="brand-mark">C</span><span>ConnectSphere</span></div><p className="nav-label">Workspace</p><nav className="nav-list" aria-label="Main navigation"><NavButton active={route === 'dashboard'} onClick={() => onNavigate('dashboard')} icon="⌂">Overview</NavButton><NavButton active={route === 'submit'} onClick={() => onNavigate('submit')} icon="＋">Submit event</NavButton><NavButton active={route === 'assignment'} onClick={() => onNavigate('assignment')} icon="◎">Coordinator</NavButton><NavButton active={route === 'review'} onClick={() => onNavigate('review')} icon="✓">Request review</NavButton><NavButton active={route === 'myEvents'} onClick={() => onNavigate('myEvents')} icon="▤">My events</NavButton></nav><div className="sidebar-footer"><span className="status-dot" /> UI baseline · mock data</div></aside>
}
