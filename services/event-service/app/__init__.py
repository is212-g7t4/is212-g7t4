...
def create_app(config=None):
    ...
    def submitted():
        ...
        if not app.config["DATABASE_URL"]:
            return jsonify(message="DATABASE_URL is not configured for Event Service."), 503
        return jsonify(events=list_submitted(app.config["DATABASE_URL"], coordinator_id))
        ...
        try:
            ...
            if request.args.get("dateTo"):
                if "T" in request.args["dateTo"]:
                    raise ValueError
                date_to = datetime.combine(datetime.fromisoformat(request.args["dateTo"]).date(), time.max)
        except ValueError:
            return jsonify(message="dateFrom and dateTo must be valid dates (YYYY-MM-DD)."), 400
        if date_from and date_to and date_to < date_from:
            return jsonify(message="dateTo must be on or after dateFrom."), 400

        if not app.config["DATABASE_URL"]:
            return jsonify(message="DATABASE_URL is not configured for Event Service."), 503
        return jsonify(events=list_events(app.config["DATABASE_URL"], coordinator_id, status, venue, date_from, date_to))
